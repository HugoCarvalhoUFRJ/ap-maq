...
A Aula~05 justifica a agregação com uma proposição de enunciado curto: se
$g_1,\dots,g_B$ são não-viesados para $r(\x)$, têm a mesma variância $v(\x)$ e são
\textbf{não-correlacionados}, então $\bar g=\frac1B\sum_b g_b$ tem variância
$v(\x)/B$.
...
\begin{itens}
  \item \pts{0,9} Identifique as duas condições e explique, para cada uma, o que
        acontece com a conclusão se ela falhar.
  \item \pts{0,7} O \emph{bagging} usa árvores \textbf{profundas e não podadas}.
        Qual das duas condições essa escolha atende? E por que podar as árvores
        seria contraproducente aqui?
  \item \pts{0,8} Na prática as árvores do \emph{bagging} são correlacionadas.
        Explique a razão --- o que elas têm em comum que as faz parecidas --- e
        por que aumentar $B$ não resolve esse problema específico.
  \item \pts{1,0} A floresta aleatória sorteia $m<p$ covariáveis candidatas em
        cada nó. Explique como isso ataca a condição comprometida, qual é o preço
        pago na outra, e por que o negócio costuma compensar mesmo assim.
\end{itens}
