...
\emph{label}) e $\x=(x_1,\dots,x_p)\in\R^p$ o vetor de \textbf{covariáveis}
...
$j$-ésima covariável na $i$-ésima observação, $\X_i=(x_{i,1},\dots,x_{i,p})$.
\end{definicao}
...
\end{tabular}
\end{center}
Aqui $n$ é o \emph{número de observações} e $p$ o \emph{número de covariáveis}.
Boa parte das dificuldades do curso aparece quando $p$ cresce em relação a $n$ ---
...
espaciais, viés de seleção, \emph{dataset shift}). Voltaremos a isso na Aula~08;
...
Entre todas as funções $g:\R^p\to\R$, a função de regressão
