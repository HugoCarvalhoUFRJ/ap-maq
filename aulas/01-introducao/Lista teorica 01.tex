...
risco $\risco(\rhat)$ & 1{,}0079 & 1{,}0392 & 0{,}6129 & 0{,}5699 & 0{,}8915 & 4{,}8503 & 79{,}2438 \\
\bottomrule
\end{tabular}

\smallskip
\begin{minipage}{0.92\textwidth}\footnotesize
O risco aqui é o \emph{erro de teste esperado}: a média sobre as $400$ amostras de
treino, com o piso $\sigma^2$ entrando exatamente, e \textbf{não} a medida num
conjunto de teste finito. A diferença é o que separa este exercício do
Exercício~2(c), e é ela que sustenta o item~(c).
\end{minipage}
\end{center}
...
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
  ...
  \item O [ISLP] define \emph{superajuste} como o caso em que \emph{um modelo menos
        ...
        da tabela há superajuste? (Cuidado: são mais do que os óbvios.) Um dos graus
        que você vai encontrar tem erro de treino \emph{alto} e variância baixíssima
        --- a tabela do Exercício~4 traz as duas parcelas dele. Explique por que,
        ainda assim, ele perde para o grau~1. \emph{Dica:} tanto
        $r(x)=\operatorname{sen}(1{,}5x)+0{,}3x$ quanto o intervalo $[-3,3]$ são
...
\smallskip
E o grau~2 merece um segundo olhar, porque ele mostra o que essa definição
\emph{não} diz. Pela tabela do Exercício~4, ali o viés$^2$ vale $0{,}4794$ contra
uma variância de $0{,}0698$: \textbf{87\% do excesso de risco é viés}. E o erro de
treino, $0{,}9192$, é o segundo mais alto da tabela. Esse é um modelo
\emph{rígido demais} --- o retrato do subajuste --- e ainda assim satisfaz o
critério de superajuste. Os dois rótulos não são exclusivos.

\smallskip
A razão de ele perder para o grau~1 é a \textbf{paridade}. Como
$r(x)=\operatorname{sen}(1{,}5x)+0{,}3x$ é uma função \emph{ímpar} e $[-3,3]$ é
simétrico em torno da origem, um termo par como $x^2$ não tem nada a aproximar:
o viés$^2$ nem cai --- sobe de leve, de $0{,}4786$ para $0{,}4794$ --- e em troca a
variância quase dobra, de $0{,}0394$ para $0{,}0698$. É um parâmetro pago e não
usado. O mesmo acontece em \emph{todo} grau par: fora da tabela, o grau~4 tem risco
$0{,}6452$ contra $0{,}6129$ do grau~3, e o grau~6 tem $0{,}6156$ contra $0{,}5699$
do grau~5. A curva em ``U'' de fato ziguezagueia; a tabela parece lisa porque pula
os graus 4, 6, 7, 9 e 11.

\smallskip
A lição é dupla. O critério do [ISLP] é \textbf{relativo} --- ele diz ``você está
do lado errado da curva'', não ``o seu modelo tem variância alta''. E ele não
diagnostica a \emph{causa}: para saber se o culpado é o viés ou a variância, só a
decomposição do Exercício~4 responde.
...
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
  \item Mostre que
        ...
        \]
        \emph{Dica:} some e subtraia $\mu:=\E_{\Dados}[\rhat(\x_0)]$ dentro do
        quadrado --- é a mesma manipulação do Exercício~2(a).
  \item Somando o erro irredutível, o risco no ponto $\x_0$ fica
        ...
        $\x_0$). Confira a identidade em \textbf{todas} as colunas da tabela, e
        descreva em uma frase o que acontece com cada parcela quando o grau cresce.
\end{enumerate}
...
grau & 1 & 2 & 5 & 10 & 12 \\
\midrule
viés$^2$    & 0{,}4786 & 0{,}4794 & 0{,}0018 & 0{,}0034 & 0{,}5410 \\
...
\]
Como a dica antecipa, é a mesma conta do item (a) do Exercício~2, com
$\rhat(\x_0)$ no lugar de $Y$ e $r(\x_0)$ no lugar de $c$ --- o que não é
coincidência: nos dois casos estamos decompondo um erro quadrático médio em torno
da média do objeto aleatório.
...
\begin{array}{lrcl}
  ...
  \text{grau } 2:  & 0{,}4794 + 0{,}0698 + 0{,}49  &=& 1{,}0392 \\
  \text{grau } 5:  & 0{,}0018 + 0{,}0781 + 0{,}49  &=& 0{,}5699 \\
  \text{grau } 10: & 0{,}0034 + 4{,}3570 + 0{,}49  &=& 4{,}8504 \quad(\approx 4{,}8503)\\
  \text{grau } 12: & 0{,}5410 + 78{,}2128 + 0{,}49 &=& 79{,}2438
\end{array}
\]
As diferenças na quarta casa são só arredondamento da tabela.

\smallskip
Comportamento das parcelas: o \textbf{viés$^2$} fica \emph{parado} do grau~1 para o
grau~2 --- o termo par não aproxima nada, que é a discussão do Exercício~3(b) --- e
só então desaba: do grau~5 em diante o polinômio já consegue reproduzir $r$, e o
viés vira ruído numérico.
