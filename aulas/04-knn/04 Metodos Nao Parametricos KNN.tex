...
antes (Aula~06). Além disso, sofre da \emph{maldição da dimensionalidade}: em $p$
alto, ``vizinhos'' deixam de ser próximos.
\end{atencao}
...
flexibilidade $\leftrightarrow$ viés--variância).

\begin{emsala}
Pergunta que rende: \emph{``o KNN é linear e tem $h_{ii}=1/k$; o atalho do LOOCV da
Aula~03 (Prop.\ do \emph{leverage}) vale para ele?''} A resposta intuitiva é sim, e
está errada. O atalho supõe que tirar a observação $i$ só reescale os pesos das
outras; no KNN, tirar $\X_i$ muda \emph{quem são} os vizinhos --- entra o
$(k+1)$-ésimo. Com $h_{ii}=1/k$, a fórmula dá
$Y_i-(\text{média dos outros } k-1 \text{ vizinhos})$: ela calcula o LOOCV do KNN com
$k-1$ vizinhos. A \texttt{Lista prática 04} mede o erro: $27\%$ em $k=2$.
\end{emsala}
