...
\emph{Padronize} antes (Aula~06). Além disso, o KNN sofre da \emph{maldição da
dimensionalidade}: em $p$ alto, ``vizinhos'' deixam de ser próximos.
\end{atencao}
...
\begin{atencao}[linear, mas sem o atalho do LOOCV]
Seria natural esperar que o atalho do LOOCV via \emph{leverage} (Aula~03, Prop.~3.1)
valesse para todo suavizador linear. Não vale, e o KNN é o contraexemplo. O atalho
supõe que tirar a observação $i$ apenas \emph{reescale} os pesos das outras. No KNN,
tirar $\X_i$ muda \emph{quem são} os vizinhos de $\X_i$: entra o $(k+1)$-ésimo, que
antes tinha peso zero.

A conta mostra o que a fórmula calcula no lugar do LOOCV. Como $h_{ii}=1/k$, e
$\rhat(\X_i)$ é a média de $Y_i$ com as respostas dos outros $k-1$ vizinhos,
\[
  \frac{Y_i-\rhat(\X_i)}{1-1/k}
  = Y_i-\big(\text{média dos outros } k-1 \text{ vizinhos de } \X_i\big),
\]
que é o resíduo de validação do KNN com $k-1$ vizinhos, e não com $k$: aplicada ao
KNN, a fórmula devolve o LOOCV do $(k-1)$-NN. O LOOCV verdadeiro é barato do mesmo
jeito --- basta pedir $k+1$ vizinhos a cada ponto de treino e descartar o próprio
---, mas não sai da fórmula. A \texttt{Lista prática 04} mede a diferença.
\end{atencao}
