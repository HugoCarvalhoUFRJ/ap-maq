...
\begin{document}
\cabecalhoaula{06}{Árvores de Regressão e \emph{Ensembles}}{AME §4.8--4.10; ISLP cap.~8}
...
\emph{cada nó}, sorteia-se um subconjunto de apenas $m<d$ covariáveis e a divisão
...
\begin{itemize}[leftmargin=1.4cm,nosep]
  \item $m$ é o \emph{tuning parameter}; regra empírica $m\approx d/3$ em regressão
        (e $m\approx\sqrt d$ em classificação). $m=d$ recupera o \emph{bagging}.
  ...
  \item Conexão com a Aula~05: a taxa de convergência das florestas depende
...
# Floresta aleatoria: B grande, m = max_features (~ d/3 em regressao)
...
\end{lstlisting}
O notebook \texttt{Aula prática 06 e 07} compara árvore, floresta e \emph{boosting}
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \item \textbf{Florestas}: \emph{descorrelacionam} sorteando $m\approx d/3$
