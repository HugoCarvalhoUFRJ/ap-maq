...
\noindent
Esta aula ataca de frente a maldição da dimensionalidade, mas pelo lado
...
\begin{itemize}[leftmargin=1.4cm,nosep]
  ...
  \item \textbf{Pré-processamento}: combater a \emph{maldição da dimensionalidade}
        e o ruído antes de um método supervisionado.
...
\begin{ideia}
O último item é o mais importante conceitualmente, e liga esta aula ao Bloco~I.
O KNN \emph{se adapta sozinho} à dimensão
intrínseca $u$, mas métodos que somam todas as coordenadas sofrem com $p$
...
\begin{figure}[htbp]
  ...
  $69$ são, em boa medida, repetição. É a \emph{redundância}, medida.}
  \label{fig:variancia}
\end{figure}
...
\emph{Padronize} as covariáveis antes (Aula~06), senão variáveis de maior magnitude
...
produtos internos, aplica-se o \textbf{truque do \emph{kernel}} (Aulas~04 e~09):
...
capturam subvariedades curvas. É literalmente o mesmo truque da Aula~09, aplicado a
...
# PCA como etapa de um pipeline (Aula 06): ajustado SO no treino
...
O PCA aprende direções a partir dos dados, então pela regra da Aula~06 ele é etapa
do modelo e tem de entrar no \emph{pipeline}. Mas vale ser preciso sobre a
\emph{gravidade}, porque a intuição engana aqui.

O que torna grave o vazamento por seleção de variáveis (Aula~06) é que a seleção
...
E a medição confirma: o notebook desta aula repete o experimento cruel da Aula~06 --
...
treino. Na hierarquia da Aula~06, portanto, o PCA fica na categoria \textbf{leve},
