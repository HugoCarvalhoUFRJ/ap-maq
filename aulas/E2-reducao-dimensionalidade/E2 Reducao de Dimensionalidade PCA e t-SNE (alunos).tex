...
apenas uma fatia fina do espaço $\R^d$, e talvez baste descrevê-la. A pergunta:

\begin{center}\itshape
  como resumir $d$ covariáveis em bem menos, sem perder o essencial?
\end{center}
...
Aula~05 vimos que o KNN e a regressão local \emph{se adaptam sozinhos} à dimensão
intrínseca $u$, mas que métodos que somam todas as coordenadas sofrem com $d$
...
\[
  Z_1 = \phi_{11}X_1+\dots+\phi_{d1}X_d = \bm{\phi}_1^\top\X
\]
...
--- parece uma sequência de $d$ otimizações encadeadas. A proposição diz que
\textbf{uma única} decomposição espectral de $\bm{C}$ resolve todas de uma vez, e
ainda ordena o resultado. É por isso que o PCA é barato mesmo com $d$ grande.
\end{ideia}
...
\begin{equation}\label{eq:pve}
  \text{PVE}(p) = \frac{\sum_{i=1}^p \lambda_i}{\sum_{i=1}^d \lambda_i}.
\end{equation}
...
O PCA também resolve: encontrar a projeção linear $\bm{Z}$ em $m<d$ dimensões que
...
O detalhe do nome tem uma razão precisa. Ao espremer dados de $\R^d$ em $\R^2$, não
...
# t-SNE: apenas para visualizar
Z = TSNE(n_components=2, perplexity=30, random_state=0).fit_transform(X)
\end{lstlisting}
...
\begin{atencao}
O PCA dentro de um \emph{pipeline} não é preciosismo: os componentes são direções
aprendidas de \emph{todas} as colunas ao mesmo tempo, e calculá-los fora da dobra é
vazamento da categoria grave da Aula~07 --- da mesma família da seleção de
variáveis que fabricou $R^2=0{,}40$ a partir de ruído.
\end{atencao}

O notebook \texttt{Exemplo - PCA} ilustra os componentes principais e a variância
