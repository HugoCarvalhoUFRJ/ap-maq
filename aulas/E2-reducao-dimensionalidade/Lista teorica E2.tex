...
Uma matriz de dados com $p=8$ covariáveis tem autovalores da matriz de
covariância
\[
...
``4 componentes'' depende de $p$ e da estrutura de correlação. Num
...
das variâncias das $p$ covariáveis originais --- a ``variância total'' dos dados.
...
$\R^p\to\R^2$; ele resolve um problema de otimização cujas variáveis são as
...
não são combinações lineares interpretáveis das originais, como as da PCA.
Alimentar um classificador com elas é usar como covariável o resultado de um
algoritmo de visualização.

Uma ressalva sobre um argumento que se lê muito a respeito do t-SNE: ``rodar de
novo com outra semente dá outro mapa'' valia quando o padrão era
\texttt{init="random"}. Desde a versão~1.2 do \texttt{scikit-learn} o padrão é
\texttt{init="pca"}, e aí o resultado é \emph{reprodutível} --- a \texttt{Aula
prática E2} mede diferença de coordenada igual a zero entre duas sementes. Com
\texttt{init="random"} as figuras mudam por completo, e mesmo assim $92{,}6\%$ das
vizinhanças de dez pontos se preservam: o mapa se embaralha, a vizinhança não.
Ou seja, a indeterminação \emph{não} é o motivo pelo qual o t-SNE fica fora de um
\texttt{Pipeline}. O motivo é a ausência de \texttt{transform}, acima.
...
Seja $\bm X$ a matriz $n\times p$ dos dados \textbf{centrados}, com decomposição
em valores singulares $\bm X = \bm U\bm D\bm V^\top$, em que $\bm U$ é
$n\times r$ com colunas ortonormais, $\bm D$ é diagonal $r\times r$ com
$d_1\ge d_2\ge\dots\ge d_r>0$, e $\bm V$ é $p\times r$ com colunas ortonormais.
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
