...
(KNN, Aula~04; árvores, Aula~05), agora para classificação. Boa notícia: a estrutura
...
\begin{description}[leftmargin=2.6cm,style=nextline]
  \item[Bagging / Florestas] treina $B$ árvores em amostras bootstrap e agrega por
        ...
        classificação: $m\approx\sqrt{p}$. Robustas a $B$; OOB e importância de
        variáveis ``de graça''.
...
A §7 da \texttt{Aula prática 10} refaz a tabela numa base de classe rara ($10\%$ de
positivos), e vale mostrar as duas medições:

\begin{itemize}[leftmargin=1.2cm,nosep]
  \item a \textbf{acurácia} varia de $0{,}8989$ a $0{,}9072$ entre os sete métodos, e
        quatro empatam \emph{exatamente} com quem responde ``não'' para todo mundo,
        porque nunca preveem positivo. A AP varia de $0{,}119$ a $0{,}453$ na mesma
        tabela;
  \item com o corte de custo $1{:}20$ da Aula~08 --- $0{,}0476$ --- a árvore podada e
        o AdaBoost mandam investigar \textbf{todos os mil por mil}, porque as
        probabilidades deles quase nunca descem abaixo de $0{,}05$. A logística manda
        $410$ e acha $82$ dos $101$ positivos; o SVM manda $493$ e acha $89$.
\end{itemize}

A moral a extrair no quadro: o corte ótimo é ótimo \emph{para probabilidade
honesta}. Pergunte à turma o que fazer com um modelo que ordena bem e calibra mal
--- a resposta é da Aula~08, e é calibrar, não trocar de modelo.
\end{emsala}
...
for nome, modelo in [("KNN", knn), ("arvore", arv), ("RF", rf), ("Ada", ada)]:
    ...
    print(nome, modelo.score(X_te, y_te))   # use tambem as metricas da Aula 08
\end{lstlisting}
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \item \textbf{Árvore de classificação}: prediz a moda \eqref{eq:arvcls}; divide
        ...
        (melhores que a taxa de erro); cresce e poda como na Aula~05.
  \item \textbf{Florestas} ($m\approx\sqrt p$) e \textbf{boosting/AdaBoost} agregam
  ...
  \item Não há classificador universalmente melhor: escolha pela estrutura/objetivo
        e compare por CV com as métricas da Aula~08.
\end{itemize}
