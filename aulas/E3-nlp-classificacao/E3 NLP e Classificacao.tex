...
Bayes ingênuo (Aula~07), métricas em dados desbalanceados (Aula~08) e
\emph{pipelines} (Aula~06). Pergunta de abertura: \emph{``um modelo só entende
...
de \textbf{classificação binária} (Aula~07) cujas covariáveis são \emph{texto bruto}
...
na Aula~08, a acurácia engana aqui --- precisaremos de precisão, \emph{recall} e
...
seleciona; a distância é dominada por palavras irrelevantes).
\end{atencao}
...
\begin{description}[leftmargin=2.8cm,style=nextline]
  \item[Bayes ingênuo (multinomial)] o ``cavalo de batalha'' do texto. A suposição
        de independência condicional (Aula~07) é grosseira para palavras, mas
        funciona surpreendentemente bem e é rápida. A versão \emph{multinomial}
        modela contagens de palavras. (Lembre da Aula~07: trabalhe em escala
...
\emph{classe} de maior probabilidade --- e, para classificar (Aula~08), basta acertar
...
\begin{itemize}[leftmargin=1.4cm,nosep]
  ...
  \item ajuste o \textbf{corte} de decisão conforme os custos (Aula~08): se um falso
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \item A matriz documento--termo é \textbf{esparsa e de alta dimensão} ---
        terreno do \textbf{Bayes ingênuo} (Aula~07) e da \textbf{logística
        penalizada}.
  \item Avalie com \textbf{precisão/\emph{recall}/$F_1$} e ajuste o \textbf{corte}
        (dados desbalanceados, Aula~08).
  \item Tudo num \textbf{\emph{pipeline}} com CV sem vazamento (Aulas~03 e~06).
\end{itemize}
