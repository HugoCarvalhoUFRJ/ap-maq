...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \item avaliar um classificador de \emph{spam} com as métricas certas (Aula~08);
  \item montar o \emph{pipeline} completo, sem vazamento (Aula~06).
\end{itemize}
...
(Aula~07) cujas covariáveis são \emph{texto bruto} --- e o desafio central é a
\emph{representação}.

\begin{atencao}
A base é \textbf{desbalanceada}. Como vimos na Aula~08, a acurácia engana aqui: um
...
o que, pela Proposição~4.1 da Aula~08, significa exigir um corte \emph{alto} para
declarar \emph{spam}.
\end{atencao}
...
\begin{ideia}
Repare na desproporção que a figura registra: $5\,572 \times 8\,672$ pá quase $48$
...
milhares de palavras que nenhuma das duas contém.
\end{atencao}

\subsection{TF--IDF}
Contagens cruas pão peso demais a palavras comuns. O \textbf{TF--IDF}
...
\begin{description}[leftmargin=2.8cm,style=nextline]
  \item[Bayes ingênuo (multinomial)] o ``cavalo de batalha'' do texto. A suposição
        de independência condicional (Aula~07) é grosseira para palavras --- ``prize''
        e ``win'' claramente andam juntas --- mas funciona surpreendentemente bem e é
        rápida. A versão \emph{multinomial} modela contagens. (Lembre da Aula~07:
...
\begin{atencao}
Mas atenção ao que a Aula~08 mediu: o Bayes ingênuo com covariáveis correlacionadas
...
\section{Avaliação}
%==============================================================================
Como o problema é desbalanceado (Aula~08):
\begin{itemize}[leftmargin=1.4cm,nosep]
...
\begin{observacao}[a métrica escolhida depois é a métrica errada]
Escolher a métrica \emph{antes} soa como formalidade, e a §8 da \texttt{Aula prática
E3} mostra que não é. Ela acrescenta ao saco de palavras duas colunas que ele não
enxerga --- o comprimento da mensagem e a proporção de dígitos ---, e que separam
muito: no treino, o \emph{ham} médio tem $70{,}9$ caracteres e $0{,}4\%$ de dígitos,
contra $138{,}8$ e $11{,}7\%$ do \emph{spam}. O resultado:
...
\toprule
 & acurácia & AUC & AP \\
\midrule
só texto          & $0{,}9821$ & $0{,}9879$ & $\mathbf{0{,}9691}$ \\
texto $+$ 2 colunas & $0{,}9862$ & $0{,}9950$ & $\mathbf{0{,}9572}$ \\
\bottomrule
\end{tabular}
\end{center}

A acurácia sobe, a AUC sobe --- e a \textbf{AP desce}. Não é contradição: são
perguntas diferentes. A AUC melhora porque as duas colunas arrumam o \emph{grosso}
do ranking, marcando com mais confiança as mensagens curtas e sem dígitos. A AP olha
a \emph{cabeça} do ranking, onde o TF--IDF já ia muito bem, e ali acrescentar duas
colunas densas ao lado de milhares de esparsas mexe nos pesos dos termos mais
informativos.

Se o filtro vai bloquear as mensagens de maior escore --- que é o caso --- quem
manda é a AP, e por ela essas duas colunas não pagam o que custam. Se a métrica
tivesse sido escolhida depois de ver os números, as três linhas justificariam três
decisões diferentes.
\end{observacao}
...
%==============================================================================
Tudo se encaixa num único \emph{pipeline} (Aula~06): vetorização $\to$ classificador,
...
categoria \textbf{grave} da Aula~06: uma etapa que \emph{escolhe} quais colunas
...
informação, e estão fora do escopo do curso ([ISLP] §10.4--10.5 pá uma introdução).
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \item Alta dimensão $+$ esparsidade $\Rightarrow$ Bayes ingênuo e logística
        penalizada; KNN vai mal.
  \item Avalie com precisão/\emph{recall}/$F_1$ e ajuste o corte pelos custos
        (Aula~08); cuidado com a calibração do Bayes ingênuo.
  \item Tudo num \emph{pipeline} (Aula~06): o vetorizador aprende o vocabulário
        \emph{só} no treino.
\end{itemize}

\noindent
E é assim que o curso fecha. Este problema não exigiu nenhuma ideia que você já não
tivesse: a representação veio da Aula~06, o classificador veio da Aula~07, a avaliação veio da Aula~08 e a montagem
