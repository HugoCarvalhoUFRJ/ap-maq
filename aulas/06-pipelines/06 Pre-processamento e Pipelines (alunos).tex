...
%  (O roteiro de sala é "06 Pre-processamento e Pipelines.tex".)
%==============================================================================
...
\begin{atencao}
Nem todo método precisa de padronização, e são \textbf{três} categorias, não duas.

Árvores, florestas e \emph{boosting} (Aula~05) comparam valores \emph{dentro} de
cada variável e são completamente indiferentes à escala --- padronizar não muda
nada além de gastar tempo. Já KNN e qualquer coisa penalizada precisam.

A terceira é menos óbvia: \textbf{LDA, QDA e Bayes ingênuo} (Aula~07) também são
invariantes, e justamente \emph{porque} estimam a covariância dos dados.
Multiplicar a coluna $j$ por $c$ multiplica a linha e a coluna $j$ de
$\widehat\Sigma$ por $c$, e a forma quadrática
$(\x-\widehat{\bm\mu})^\top\widehat\Sigma^{-1}(\x-\widehat{\bm\mu})$ sai idêntica
--- a padronização já está embutida na estimativa. Isso vale para os estimadores
de máxima verossimilhança. No \texttt{scikit-learn}, o \texttt{GaussianNB} soma às
variâncias um termo proporcional à maior delas, e o \texttt{reg\_param} do QDA puxa
a covariância na direção da identidade: os dois dependem da escala e quebram a
invariância exata.

Saber \emph{por que} cada método precisa (ou não) é melhor do que padronizar por
reflexo --- ainda que, na dúvida, deixar o \texttt{StandardScaler} no
\emph{pipeline} seja barato: nos invariantes ele não custa nada.
\end{atencao}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{../../recursos/figuras/06-vazamento.pdf}
  \caption{Dois vazamentos, medidos em 40 repetições, com validação cruzada de 5
  dobras. \textbf{À esquerda}, seleção de variáveis: $n=60$ observações, $p=3000$
...
\begin{enumerate}[leftmargin=1.6cm,nosep]
  \item \textbf{Grave:} escolher variáveis, hiperparâmetros ou o próprio modelo
        olhando dados que depois serão usados para avaliar. Inflação sem limite,
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[font=\small, x=1cm, y=1cm,
    ...
    \node[cx,fill=cinzaclaro] (p1) at (0.9,0)  {dados};
    ...
    \foreach \a/\b in {p1/p1, p1/s1, s1/a1, a1/m1} \draw[seta] (\a) -- (\b);
    ...
    \node[cx,fill=cinzaclaro] (p2) at (0.9,-3.3)  {dados};
    ...
    \foreach \a/\b in {p2/s2, s2/p2, p2/m2} \draw[seta] (\a) -- (\b);
...
O notebook \texttt{Aula prática 06} segue o fio principal desta aula: mede quais
métodos são sensíveis à escala, põe um número no custo de padronizar fora da dobra
(chega a $0{,}8342$ contra $0{,}8341$, com outra semente, e a mesma conclusão do
painel direito da Figura~\ref{fig:vazamento}), abre o \texttt{Pipeline} para
conferir que o scaler aprendeu a média \emph{do treino}, e fecha entregando o
\emph{pipeline} inteiro ao \texttt{GridSearchCV}. A \texttt{Lista prática 06}
exercita o mesmo caminho, com outras sementes.

O vazamento por seleção de variáveis --- o painel esquerdo da
Figura~\ref{fig:vazamento} --- e o vazamento por observações agrupadas não têm
laboratório próprio: ficam por conta destas notas, e do Exercício~1 da
\texttt{Lista teorica 06}, onde aparecem entre os seis casos a classificar. São os
dois \textbf{graves} da hierarquia, e vale reler esta seção com cuidado justamente
porque você não vai medi-los com as mãos.
...
\paragraph{Para praticar.} \texttt{Lista teorica 06.pdf} (teórica, com gabarito) e \texttt{Lista prática 06.ipynb} (prática, para completar as lacunas), nesta mesma pasta.
