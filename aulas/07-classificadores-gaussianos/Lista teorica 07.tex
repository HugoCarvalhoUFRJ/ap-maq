...
Em $p=2$ os dois métodos são praticamente equivalentes em toda a tabela: a maior
...
Em $p=10$ os dois erram feio, em direções opostas. Com $n=20$, o QDA precisa
...
$p(p+1)/2$}, que é o custo da matriz extra. Segundo, e mais útil na prática:
\textbf{em dimensão baixa a escolha quase não importa}; ela só passa a importar
quando $p$ é grande, e aí importa muito, nos dois sentidos. Gastar validação
cruzada decidindo entre LDA e QDA em $p=2$ é desperdício; em $p=50$ é obrigatório.
...
eles. Some-se a isso o argumento de variância do item (a): com $4p+1$ parâmetros
