...
\textbf{erra mais da metade dos positivos} ($0{,}5393$ deles!) --- e ainda assim é
...
\[
  ...
  = \ln\pi_k - \tfrac{p}{2}\ln(2\pi) - \tfrac12\ln\abs{\bm\Sigma}
...
$-\tfrac p2\ln(2\pi)$, $-\tfrac12\ln\abs{\bm\Sigma}$ e
...
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
  ...
  \item A tabela abaixo traz a acurácia medida (média sobre 60 amostras, avaliada
        em 20 mil observações novas) do LDA e do QDA em $p=2$ e $p=10$; o traço
        marca onde o QDA não pôde ser ajustado. Nas duas dimensões o QDA passa a
        ganhar a partir de $n=50$ --- mas o que acontece \emph{antes} disso é
        muito diferente. Use a contagem de (b) para explicar, inclusive o traço, e
        diga por que a regra ``prefira LDA com poucos dados'' está incompleta.
\end{enumerate}
...
em cada uma, uma matriz com $55$ parâmetros. A covariância amostral de $10$ pontos
em $\R^{10}$ tem posto no máximo $9$ --- não é invertível ---, e o QDA simplesmente
não pode ser ajustado. Com $n=30$ ele já ajusta, mas fica $3{,}9$ pontos atrás do
LDA. Com $n=1000$ ele reproduz a estrutura verdadeira e ganha $6{,}6$ pontos.
...
\emph{perder}; e, nesta população, as duas classes têm quase a mesma correlação,
então ele também tem pouco a \emph{ganhar}. Isso não é uma propriedade de $p=2$:
numa população em $\R^2$ com correlações de sinais opostos, o QDA ganha do LDA com
folga desde $n=20$ --- é o que mede a \texttt{Aula prática 07}.
...
$p(p+1)/2$}, que é o custo da matriz extra --- e esse custo só pesa quando $p$ é
grande. Segundo, a regra fala só do que o QDA tem a perder: \textbf{o que ele tem
a ganhar depende de quanto as covariâncias das classes diferem}, e isso nenhuma
contagem de parâmetros informa --- é assunto para a validação cruzada.
...
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
  ...
  \item Mostre que a função discriminante do naive Bayes gaussiano é
        \[
          \delta_k(\x) = \ln\pi_k
            - \tfrac12\sum_{j=1}^{p}\left[\ln\sigma_{kj}^2
...
\begin{solucao}
\textbf{(a)} São $2p$ médias, $2p$ variâncias (uma por covariável e por classe) e
$1$ priori: $\#\text{NB}=4p+1$. Comparando:
\begin{center}\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}lrrr@{}}
\toprule
$p$ & $2$ & $10$ & $50$ \\
\midrule
naive Bayes ($4p+1$) & $9$  & $41$  & $201$ \\
LDA                  & $8$  & $76$  & $1\,376$ \\
QDA                  & $11$ & $131$ & $2\,651$ \\
\bottomrule
\end{tabular}
\end{center}
Note a inversão: em $p=2$ o naive Bayes custa \emph{mais} que o LDA (9 contra 8),
porque estima duas variâncias por covariável em vez de uma covariância comum. Em
$p=50$ ele custa $201$ contra $1\,376$ --- quase sete vezes menos. O crescimento
é \textbf{linear} em $p$, contra o quadrático dos outros dois, e é isso que o
torna viável em dimensão alta (a Aula~E3 usa a versão multinomial com quase nove
mil colunas).

\smallskip
\textbf{(b)} Com $\bm\Sigma_k$ diagonal, a densidade conjunta fatora:
$f_k(\x)=\prod_{j=1}^p f_{kj}(x_j)$, com
$f_{kj}$ a densidade de $N(\mu_{kj},\sigma_{kj}^2)$. Então
\[
  \ln\big(\pi_k f_k(\x)\big)
   = \ln\pi_k + \sum_{j=1}^{p}\left[
       -\tfrac12\ln(2\pi) - \tfrac12\ln\sigma_{kj}^2
       - \frac{(x_j-\mu_{kj})^2}{2\sigma_{kj}^2}\right],
\]
e descartando $-\tfrac p2\ln(2\pi)$, que é comum a todas as classes, chega-se à
...
maior, e não estimar bem as probabilidades. Com duas classes, a decisão depende só
do sinal de $\delta_1-\delta_0$; quando covariáveis correlacionadas apontam para a
mesma classe, contá-las como independentes exagera o \emph{tamanho} dessa diferença
sem trocar o seu sinal. Some-se a isso o argumento de variância do item (a): o naive
Bayes estima $4p+1$ parâmetros, contra $2p+p(p+1)+1$ do QDA --- $201$ contra $2\,651$
em $p=50$ ---, então as estimativas são muito mais estáveis, e o viés introduzido pela
suposição falsa é compensado pela variância que se economiza.
...
informação várias vezes. É exatamente a medição da Aula~08 --- AUC praticamente
...
Um exemplo em $p=2$: suponha que nas duas classes cada covariável tenha média
