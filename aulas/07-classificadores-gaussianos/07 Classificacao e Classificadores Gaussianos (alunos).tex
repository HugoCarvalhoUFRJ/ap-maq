...
na Aula~09 e em qualquer rede neural.
\end{ideia}
...
estimar uma densidade em $\R^p$ --- tarefa condenada pela maldição da
dimensionalidade --- estimam-se $p$ densidades em $\R^1$. É a
...
QDA é mais flexível (menos viés) mas estima muito mais parâmetros. A conta: em $p$
dimensões, LDA estima \emph{uma} matriz de covariância, com $p(p+1)/2$ entradas
livres; QDA estima \emph{uma por classe}. Com $p=10$ e duas classes, são $55$
...
5 e 2. Regra de bolso: LDA com poucos dados ou $p$ grande, QDA com muitos dados.
...
Aula~08 antes de tirar conclusões dessas quatro linhas.
\end{atencao}
