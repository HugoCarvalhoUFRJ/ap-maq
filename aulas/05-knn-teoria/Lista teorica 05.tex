...
\]
Suponha que, em $p=1$, $n=100$ observações bastem para atingir um risco alvo
$\delta$ (tratando a desigualdade como igualdade).
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
  \item Isolando $n$, mostre que $n=(K'/\delta)^{(2+p)/2}$.
  \item Use o dado de $p=1$ para achar $K'/\delta$ e calcule quantas observações
        seriam necessárias, para o \emph{mesmo} risco, em $p=2$, $p=5$, $p=10$ e
        $p=20$.
  \item O crescimento é polinomial ou exponencial em $p$? Justifique olhando a
        fórmula.
...
\begin{solucao}
\textbf{(a)} De $\delta=K'n^{-2/(2+p)}$ vem $n^{2/(2+p)}=K'/\delta$ e, elevando
a $(2+p)/2$,
\[
  n = \Big(\frac{K'}{\delta}\Big)^{(2+p)/2}.
\]

\smallskip
\textbf{(b)} Em $p=1$ o expoente é $3/2$, então
...
\end{tabular}
\end{center}
Cem observações em $p=1$ viram \textbf{cem milhões} em $p=10$. Em $p=20$ seriam
$4{,}6\times10^{14}$, cerca de sessenta mil vezes a população da Terra.

\smallskip
\textbf{(c)} \textbf{Exponencial.} O $p$ aparece no \emph{expoente}: escrevendo
$c=K'/\delta$, temos $n=c^{(2+p)/2}=c\cdot(\sqrt{c}\,)^{p}$. Cada dimensão nova
multiplica $n$ por um fator constante $\sqrt{c}$ --- aqui, $\sqrt{21{,}54}\approx
4{,}64$. É exatamente o que a tabela mostra: de $p=10$ para $p=20$, dez
...
$n^{-2/(2+p)}$ diz a velocidade --- e em $p=20$ o expoente é $-1/11$, de modo que
...
Espalhe $n$ pontos uniformemente no cubo $[0,1]^p$. Para que uma bola de raio
$\rho$ em torno de um ponto contenha em média $k$ deles, é preciso
$n\rho^p\approx k$, ou seja
\[
  \rho \;\approx\; \Big(\frac{k}{n}\Big)^{1/p} .
\]
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
  \item Com $k=10$ e $n=10\,000$, calcule $\rho$ para $p=1$, $2$, $5$, $10$ e
        $20$. (Repare que $k/n=0{,}001$ em todos os casos.)
  \item Interprete o valor de $p=20$ em palavras: que fração da largura do cubo
        o raio cobre? O que resta da palavra ``vizinho''?
  \item Ainda em $p=20$, quantas observações seriam necessárias para que os $10$
...
\begin{solucao}
\textbf{(a)} Com $k/n=10^{-3}$, temos $\rho=(10^{-3})^{1/p}=10^{-3/p}$:
\begin{center}\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}lccccc@{}}
\toprule
...
\smallskip
\textbf{(b)} Em $p=20$ o raio necessário é $0{,}708$: a bola cobre cerca de
...
Ou seja, o defeito que em $p=1$ é localizado nas duas pontas passa, em $p$
...
\[
  \E\big[(\rhat_k(\X)-r(\X))^2\big]
   \;\le\; \underbrace{K\Big(\frac{k}{n}\Big)^{2/p}}_{\text{viés}^2}
...
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
  ...
  \item Minimize a soma em $k$ e mostre que o ótimo satisfaz
        $k \asymp n^{2/(2+p)}$. \emph{Sugestão:} derive em $k$ e iguale a zero, ou
        simplesmente iguale os dois termos --- a menos de constantes dá o mesmo.
  \item Substitua de volta e obtenha a taxa $n^{-2/(2+p)}$.
  \item O que acontece com o $k$ ótimo quando $p$ cresce, com $n$ fixo? Comente o
...
se parece com $r(\x)$ --- é o fator $(k/n)^{2/p}$, que é o raio ao quadrado. A
...
se cruzam):
\[
  \Big(\frac{k}{n}\Big)^{2/p} \asymp \frac1k
  \;\Longrightarrow\;
  k^{2/p}\,k \asymp n^{2/p}
  \;\Longrightarrow\;
  k^{(2+p)/p} \asymp n^{2/p}
  ...
  k \asymp n^{2/(2+p)} .
\]
Pela derivada dá o mesmo: $\frac{p}{dk}\big[Kn^{-2/p}k^{2/p}+\sigma^2k^{-1}\big]
= \frac{2K}{p}n^{-2/p}k^{2/p-1}-\sigma^2k^{-2}=0$, o que leva a
$k^{2/p+1}=\frac{p\sigma^2}{2K}n^{2/p}$ e à mesma ordem.

\smallskip
\textbf{(c)} Levando $k\asymp n^{2/(2+p)}$ ao termo de variância (o mais fácil
dos dois):
\[
  \frac{\sigma^2}{k} \asymp \sigma^2\,n^{-2/(2+p)} .
\]
No termo de viés dá o mesmo, por construção --- foi assim que escolhemos $k$.
Logo a soma é da ordem de $n^{-2/(2+p)}$.

\smallskip
\textbf{(d)} O expoente $2/(2+p)$ vai a zero quando $p\to\infty$, então
$k\asymp n^{2/(2+p)}\to n^0=$ constante. Ou seja: em dimensão alta, o $k$ ótimo
...
\[
  r(\x) = f_1(x_1) + f_2(x_2) + \dots + f_p(x_p),
\]
cada $f_j$ é uma função de \textbf{uma} variável e pode ser estimada à taxa
unidimensional $n^{-2/3}$. Somando os $p$ erros, o risco fica da ordem de
$p\,n^{-2/3}$.
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
  \item Com $p=10$ e $n=10\,000$, calcule as duas taxas: $n^{-2/(2+p)}$ (sem
        suposição) e $p\,n^{-2/3}$ (aditiva). Qual a razão entre elas?
  \item Quantas observações o método sem suposição precisaria para alcançar o
        erro que o aditivo alcança com $10\,000$?
  \item Onde entra $p$ em cada uma das duas taxas? É essa a diferença essencial.
...
\begin{solucao}
\textbf{(a)} Com $n=10^4$ e $p=10$:
\[
  n^{-2/(2+p)} = (10^4)^{-1/6} = 10^{-2/3} \approx \mathbf{0{,}215},
  \qquad
  p\,n^{-2/3} = 10\times(10^4)^{-2/3} = 10\times 10^{-8/3} \approx \mathbf{0{,}0215}.
\]
...
\smallskip
\textbf{(c)} Na taxa sem suposição, $p$ está no \textbf{expoente}:
$n^{-2/(2+p)}$. É por isso que ela degrada exponencialmente. Na taxa aditiva, $p$
é um \textbf{fator multiplicativo}: $p\cdot n^{-2/3}$, com expoente $-2/3$ fixo,
o mesmo de um problema unidimensional. Dobrar $p$ dobra o erro, em vez de exigir
