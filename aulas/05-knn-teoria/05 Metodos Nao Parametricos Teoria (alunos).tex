...
de $n$ (e da dimensão $d$, da suavidade etc.). Por exemplo, uma taxa $n^{-1/2}$ é
...
Se $\X$ é limitado e $d\ge 3$, então
\[
  \E\!\left[\Big(\frac1k\sum_{i\in\mathcal{N}_{\X}}\norm{\X_i-\X}\Big)^2\right]
  \le C\,\Big(\frac{k}{n}\Big)^{2/d}.
\]
\end{lema}

\begin{ideia}[de onde vem esse $2/d$]
O expoente não é mágica; é volume. Numa bola de raio $\rho$ em $\R^d$, o volume
cresce como $\rho^d$. Se os $n$ pontos estão espalhados num conjunto de volume $1$,
uma bola de raio $\rho$ contém, em média, $\approx n\rho^d$ deles. Para que ela
contenha os $k$ vizinhos que queremos, precisamos de $n\rho^d\approx k$, ou seja
\[
  \rho \;\approx\; \Big(\frac{k}{n}\Big)^{1/d}.
\]
Elevando ao quadrado (porque o lema fala do quadrado da distância) sai o
$(k/n)^{2/d}$. \textbf{Todo o resto da aula é consequência desse expoente $1/d$.}
\end{ideia}

\noindent
Em palavras: quanto mais vizinhos ($k$) eu exijo, mais \emph{longe} eles ficam. E o
$1/d$ no expoente diz que, em dimensão alta, essa distância cresce de forma
brutal --- a raiz $d$-ésima de um número pequeno está perto de $1$.

\begin{exemplo}[um número para sentir o problema]
Tome $n=10\,000$ pontos e $k=10$ vizinhos, num cubo de lado $1$. Então
$\rho\approx (10/10\,000)^{1/d} = (0{,}001)^{1/d}$:
\begin{center}\small
...
\end{tabular}
\end{center}
Em $d=1$, os dez vizinhos cabem num intervalo de comprimento $0{,}001$: são
\emph{de fato} vizinhos. Em $d=20$, para reunir dez pontos você precisa de uma bola
...
\[
  \E\big[(\rhat_k(\X)-r(\X))^2\big]
   \;\le\; K\,\Big(\frac{k}{n}\Big)^{2/d} + \frac{\sigma^2}{k}.
\]
...
$\E[(\rhat_k(\X)-r(\X))^2]\le K(k/n)^{2/d}+\sigma^2/k$. Para minimizar em $k$,
iguale (a menos de constantes) os dois termos: $(k/n)^{2/d}\asymp 1/k$, isto é,
$k^{1+2/d}\asymp n^{2/d}$, ou seja $k\asymp n^{2/(2+d)}$, e substituindo obtém-se
a taxa $n^{-2/(2+d)}$.
\end{proof}

\begin{ideia}
A prova exibe explicitamente o balanço viés--variância da Aula~01: o termo
$(k/n)^{2/d}$ \emph{cresce} com $k$ (viés --- vizinhos mais distantes, respostas
...
(Aula~03) é a maneira prática de encontrá-lo sem conhecer $L$, $\sigma^2$ nem $d$.
\end{ideia}
...
%==============================================================================
Olhe a taxa minimax $n^{-\alpha/(\alpha+d)}$: para $d$ grande, o expoente
$\alpha/(\alpha+d)$ se aproxima de $0$ --- a convergência fica \emph{lentíssima}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{../../recursos/figuras/05-taxas.pdf}
  \caption{À esquerda, a taxa $n^{-2/(2+d)}$ em escala log--log: em $d=1$ a reta
  desce com inclinação $-2/3$; em $d=20$, com $-1/11$ --- quase horizontal. Aumentar
  a amostra deixa de adiantar. À direita, a leitura prática: quantas observações são
  necessárias para atingir risco $0{,}05$. Em $d=1$ bastam $90$; em $d=10$ já são
  $6{,}4\times10^{7}$; em $d=20$, $2\times10^{14}$ --- vinte e cinco mil vezes a
  população da Terra.}
  \label{fig:taxas}
\end{figure}

\begin{teorema}[Tamanho de amostra explode com $d$]
Para garantir risco $\le\delta$ na classe $\mathcal{S}$, é necessário
\[
  n \;\gtrsim\; \Big(\frac{K}{\delta}\Big)^{(\alpha+d)/\alpha},
\]
um crescimento \textbf{exponencial} em $d$.
\end{teorema}
...
$\R^d$ com $d$ grande, qualquer vizinhança de raio fixo tem volume
desprezível: com alta probabilidade \emph{não há nenhum dado dentro dela}. Para
capturar $k$ vizinhos preciso de um raio $\asymp (k/n)^{1/d}$, que cresce com $d$
...
num cubo $[0,1]^d$ e veja o quão longe fica o vizinho mais próximo de um ponto
...
\begin{figure}[htbp]
  ...
  $[0,1]^d$, medida como fração do diâmetro do cubo. Em $d=1$ ela é praticamente
  zero; em $d=10$ já é $16\%$ do diâmetro; em $d=100$, um terço. Dito de outro modo:
...
$S\subset\{1,\dots,d\}$ de covariáveis:
\[
  r(\x) = r\big((x_i)_{i\in S}\big),\qquad \abs{S}=s \ll d.
\]
Embora haja $d$ covariáveis, só $s$ importam. Sob esparsidade a dimensão
...
\begin{itemize}[leftmargin=1.4cm]
  \item Métodos que \textbf{selecionam variáveis} --- Lasso (Aula~02), árvores e
        ...
        a operar na dimensão efetiva $s$. Para eles, o $d$ gigante quase não custa.
...
essa aposta está errada, ele paga o preço integral da dimensão $d$, mesmo que a
...
A hipótese de \textbf{redundância} diz que as covariáveis, embora em número $d$,
...
$n^{-4/(4+d)}$ quando $u\ll d$. SVR com kernel gaussiano e séries \emph{espectrais}
...
$1/d$ do raio que reúne $k$ vizinhos, estima a taxa de convergência do KNN em
$d=1,2,4,8$, abre o risco em viés$^2$ e variância para ver as duas parcelas se
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \item KNN: risco $\le K(k/n)^{2/d}+\sigma^2/k$, ótimo em
        $k\asymp n^{2/(2+d)}$, taxa $n^{-2/(2+d)}$ (Teo.~\ref{teo:knn}). O expoente
        $1/d$ nasce do volume da bola em $\R^d$ (Lema~\ref{lem:viz}).
  \item Séries ($m$-suaves): taxa $n^{-2m/(2m+1)}$; ambas atingem a taxa
        \emph{minimax} $n^{-\alpha/(\alpha+d)}$ --- ou seja, não há método melhor.
  \item \textbf{Maldição da dimensionalidade}: a taxa degrada com $d$
...
\paragraph{Para praticar.} \texttt{recursos/listas/Lista de exercícios 05.pdf}.
