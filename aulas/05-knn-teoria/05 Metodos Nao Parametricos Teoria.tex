...
essa distância cresce com $(k/n)^{1/d}$. O expoente $1/d$ é o gérmen da maldição da
dimensionalidade.
...
$(k/n)^{2/d}$ \emph{cresce} com $k$ (viés) e $\sigma^2/k$ \emph{decresce} com $k$
...
desta aula: o expoente $1/d$ do Lema, a taxa do Teorema, e a diferença entre
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \item KNN: risco $\le K(k/n)^{2/d}+\sigma^2/k$, ótimo em
        $k\asymp n^{2/(2+d)}$, taxa $n^{-2/(2+d)}$ (Teo.~\ref{teo:knn}).
  \item Séries ($m$-suaves): taxa $n^{-2m/(2m+1)}$; ambas atingem a taxa
        \emph{minimax} $n^{-\alpha/(\alpha+d)}$.
  \item \textbf{Maldição da dimensionalidade}: a taxa degrada com $d$ e o $n$
