...
\begin{atencao}[o que a medição mostra --- e o que não mostra]
A \texttt{Aula prática 03} (§8) mede as duas coisas, em 300 amostras de $n=50$. O
viés se comporta como previsto: cai de $0{,}43$ em $k=2$ para $0{,}006$ em $k=10$.
Já o desvio-padrão \textbf{não} cresce com $k$: vai de $2{,}20$ ($k=2$) a $0{,}13$
(LOOCV), decrescendo o tempo todo --- e com $n=200$ continua a descer
($0{,}068 \to 0{,}057$), então não é artefato de amostra pequena. O efeito de
treinar com poucos dados domina de longe o da correlação entre dobras.
\textbf{Não passe ``LOOCV tem variância alta'' como lei:} é um argumento sobre um
dos termos, e o outro pode ser maior.
\end{atencao}

\begin{emsala}
Vale expor a contradição em vez de escondê-la: apresente o argumento clássico,
pergunte o que a turma espera do desvio-padrão em função de $k$, e só então mostre
a medição. É um bom momento para dizer que argumento sobre um termo não é
conclusão sobre a soma.
\end{emsala}

\noindent A conclusão prática sobrevive, só que por outro motivo: de $k=5$ em
diante não há praticamente nada a ganhar em precisão, e o custo continua subindo.
Daí $k=5$ ou $k=10$.
