...
No MQO, nos \emph{splines} e no Ridge com $\lambda$ fixo, vale
\[
  \riscohat_{\text{LOO}}
   = \frac1n \sum_{i=1}^n \left( \frac{Y_i-\widehat{g}(\X_i)}{1-h_{ii}} \right)^2,
\]
onde $\widehat g$ é ajustado com \emph{todos} os dados e $h_{ii}$ é o $i$-ésimo
elemento diagonal (\emph{leverage}) da matriz $\bm{H}$ que leva as respostas
$Y_1,\dots,Y_n$ aos valores ajustados $\widehat g(\X_1),\dots,\widehat g(\X_n)$. No
MQO, $\bm{H}=\bm{X}(\bm{X}^\top\bm{X})^{-1}\bm{X}^\top$ é a matriz de projeção, e
nos \emph{splines} vale o mesmo, com as funções da base nas colunas de $\bm{X}$; no
Ridge, a inversa do meio é a da solução fechada da Aula~02, e $\bm{H}$ deixa de ser
uma projeção.
\end{proposicao}
...
isso que usamos \emph{pipelines}, Aula~06.)
\end{atencao}
...
florestas aleatórias (Aula~05), onde as observações OOB dão uma estimativa de risco
