...
variância. Vamos começar pela pergunta que motiva tudo:
...
\end{equation}
onde $\mathbb{X}\in\R^{n\times(p+1)}$ é a matriz de delineamento (linha $i$ igual a
$\x_i^\top$, com a coluna de $1$s) e $\bm{y}=(y_1,\dots,y_n)^\top$.

\begin{proposicao}[Equações normais]\label{prop:normal}
Se $\mathbb{X}^\top\mathbb{X}$ é invertível, o minimizador de \eqref{eq:mqo} é
...
\begin{proof}
Seja $S(\bbeta)=\norm{\bm{y}-\mathbb{X}\bbeta}^2
=\bm{y}^\top\bm{y}-2\bbeta^\top\mathbb{X}^\top\bm{y}+\bbeta^\top\mathbb{X}^\top\mathbb{X}\bbeta$.
O gradiente é $\nabla S(\bbeta)=-2\mathbb{X}^\top\bm{y}+2\mathbb{X}^\top\mathbb{X}\bbeta$.
Igualando a zero obtêm-se as \emph{equações normais}
$\mathbb{X}^\top\mathbb{X}\,\bbeta=\mathbb{X}^\top\bm{y}$; como a Hessiana
$2\mathbb{X}^\top\mathbb{X}$ é semidefinida positiva, o ponto crítico é mínimo. Se
$\mathbb{X}^\top\mathbb{X}$ é invertível, vale \eqref{eq:betahat}.
\end{proof}
...
\end{teorema}
\begin{proof}[Esboço]
Minimizando $\E[(y-\bbeta^\top\x)^2]$ obtém-se $\bbeta^*=\Sigma^{-1}\bm{\alpha}$, com
...
$\widehat\Sigma\to\Sigma$ e $\widehat{\bm{\alpha}}\to\bm{\alpha}$; o teorema de Mann--Wald
transporta a convergência através da função contínua
$(\Sigma,\bm{\alpha})\mapsto\Sigma^{-1}\bm{\alpha}$, e de novo através de
...
de especificação --- ao custo de aumentar $p$, que é exatamente o problema da próxima
seção.
\end{ideia}
...
\begin{enumerate}[leftmargin=1.6cm]
  \item \textbf{Variância alta.} Quanto mais parâmetros, maior $\Var(\widehat{\bbeta})$
        ...
        =\sigma^2(\mathbb{X}^\top\mathbb{X})^{-1}$ ``explode'' quando as colunas de
        $\mathbb{X}$ são quase colineares.
  \item \textbf{Não unicidade.} Se $p>n$, então $\mathbb{X}^\top\mathbb{X}$ é singular e
...
Vale ver a primeira delas acontecer. Fixe $n=60$ observações e vá aumentando $p$, com
...
\begin{figure}[htbp]
  ...
  \caption{Com $n=60$ fixo e $p$ crescendo. À esquerda, o número de condição de
  $\mathbb{X}^\top\mathbb{X}$ --- a razão entre seu maior e seu menor autovalor --- sai de
...
\begin{atencao}[o caso $p>n$ é qualitativamente diferente]
Repare que os problemas 1 e 2 não são o mesmo. Se $p<n$ mas as colunas são quase
colineares, a solução \emph{existe e é única}, só que instável. Se $p>n$, ela nem é
única: o núcleo de $\mathbb{X}$ tem dimensão $\ge p-n$, e somar a $\widehat{\bbeta}$
...
A economia do \emph{stepwise} é enorme: ele examina $1+p(p+1)/2$ modelos em vez de
$2^p$. Para $p=20$ isso é $211$ contra $1\,048\,576$; para $p=40$, $821$ contra
mais de $10^{12}$.
...
\begin{equation}\label{eq:ridgesol}
  \widehat{\bbeta}^{\,\text{ridge}}
   = (\mathbb{X}^\top\mathbb{X} + \lambda \mathbb{I}_0)^{-1}\mathbb{X}^\top\bm{y},
\end{equation}
onde $\mathbb{I}_0$ é a identidade $(p+1)\times(p+1)$ \emph{com o primeiro elemento
...
Para todo $\lambda>0$, a matriz $\mathbb{X}^\top\mathbb{X}+\lambda\mathbb{I}$ é definida
positiva (logo invertível), mesmo quando $p>n$.
\end{proposicao}
\begin{proof}
Para qualquer $\bm{v}\neq 0$,
$\bm{v}^\top(\mathbb{X}^\top\mathbb{X}+\lambda\mathbb{I})\bm{v}
=\norm{\mathbb{X}\bm{v}}^2+\lambda\norm{\bm{v}}^2\ge \lambda\norm{\bm{v}}^2>0$.
\end{proof}
...
$\mathbb{X}^\top\mathbb{X}$ de perto de zero para pelo menos $\lambda$, e o número de
...
\begin{observacao}[Por que o Lasso aguenta $p$ enorme]
...
\[
  ...
  \;\le\; \sqrt{\frac{16(L+1)^4B^2}{n}\,\log\!\Big(\frac{2p}{\delta}\Big)} .
\]
O detalhe que importa é onde o $p$ aparece: dentro de um \textbf{logaritmo}. Dobrar
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=1.42, >=Stealth]
    ...
    \begin{scope}
      ...
      \node[font=\scriptsize,vinho,below right=1pt and -3pt] at (0.4846,0) {$(0{,}48\,;\,0)$};
...
numa aresta e nenhum coeficiente seria zerado. Em dimensão $p$, porém, o losango tem
$2p$ quinas e $2^p$ faces de dimensão baixa, e a chance de acertar alguma cresce
...
Quando $\mathbb{X}^\top\mathbb{X}=\mathbb{I}$, os estimadores têm forma fechada em função do
MQO $\widehat\beta_j$:
\[
...
conhecido. Então:
\begin{itemize}[leftmargin=1.4cm]
  \item se $\bbeta\sim N(0,\sigma_\beta^2\mathbb{I})$, a \textbf{moda a posteriori} de
        $\bbeta$ é exatamente a solução Ridge \eqref{eq:ridge} com
        $\lambda=\sigma^2/\sigma_\beta^2$;
  \item se $\beta_1,\dots,\beta_p\stackrel{\text{i.i.p.}}{\sim}
...
conecta diretamente com a Aula~06 (\emph{pipelines} e \texttt{StandardScaler}).
\end{atencao}
...
\begin{figure}[htbp]
  ...
  \caption{Coeficientes em função da penalização, num problema com $n=80$, $p=30$ e
...
\emph{pipeline} (Aula~06) para evitar vazamento.
\begin{lstlisting}
...
(\texttt{superconductivity.csv}, $p=81$): mede o número de condição, exibe duas
soluções distintas de MQO com o mesmo RSS quando $p>n$, levanta os caminhos de
regularização e compara MQO, Ridge e Lasso.

\begin{observacao}[quando a regularização \emph{não} ajuda]
Vale saber de antemão o que você vai encontrar lá: com $n=14\,884$ e $p=81$, o MQO
vai bem e o Lasso \emph{não} melhora a predição --- as amostras sobram para estimar
81 parâmetros com precisão. O ganho aparece em outro lugar: o Lasso entrega quase o
mesmo desempenho (EQM de teste $318{,}8$ contra $307{,}8$) com 66 covariáveis em vez
de 81. Já num recorte com $n=40$, os papéis se
invertem e a regularização ganha com folga. Regularizar não é sempre melhor; é melhor
quando $p$ é grande \emph{em relação a} $n$.
\end{observacao}
...
\begin{itemize}[leftmargin=1.4cm]
  \item Regressão linear: $g(\x)=\bbeta^\top\x$, linear \emph{nos parâmetros};
        MQO $\widehat{\bbeta}=(\mathbb{X}^\top\mathbb{X})^{-1}\mathbb{X}^\top\bm{y}$. Mesmo com
  ...
  \item \textbf{Lasso} ($\ell_1$): faz seleção de variáveis pela geometria das
        ...
        (Fig.~\ref{fig:soft}); a garantia teórica depende de $p$ apenas como
        $\log p$.
...
\paragraph{Para praticar.} \texttt{Lista teorica 02.pdf} e \texttt{Lista prática 02.ipynb}
