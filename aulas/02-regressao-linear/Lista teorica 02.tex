...
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
  \item Para a Ridge, use a solução fechada
        $\widehat{\bbeta}^{\,\text{ridge}}=(\mathbb{X}^\top\mathbb{X}+\lambda\mathbb{I})^{-1}\mathbb{X}^\top\bm y$
...
\begin{solucao}
\textbf{(a)} Quando $\lambda\to 0$, $(\mathbb{X}^\top\mathbb{X}+\lambda\mathbb{I})^{-1}\to
(\mathbb{X}^\top\mathbb{X})^{-1}$ (se esta existir) e a Ridge tende ao MQO: viés zero sob o
modelo linear correto, e variância máxima. Quando $\lambda\to\infty$, a matriz
$\lambda\mathbb{I}$ domina, o inverso vai a zero e
...
assunto da Aula~06 e o motivo de existir o \texttt{Pipeline}.
\end{solucao}

%------------------------------------------------------------------------------
\begin{exercicio}[Ridge e Lasso com números]
No caso ortonormal ($\mathbb{X}^\top\mathbb{X}=\mathbb{I}$) as duas soluções têm forma fechada em
função do MQO:
\[
...
\begin{enumerate}[label=(\alph*),leftmargin=1.1cm]
  \item Minimizando
        $S_\lambda(\bbeta)=\norm{\bm y-\mathbb{X}\bbeta}^2+\lambda\norm{\bbeta}^2$,
        obtenha
        $\widehat{\bbeta}^{\,\text{ridge}}=(\mathbb{X}^\top\mathbb{X}+\lambda\mathbb{I})^{-1}\mathbb{X}^\top\bm y$.
        \emph{Sugestão:} o gradiente de $\norm{\bm y-\mathbb{X}\bbeta}^2$ está na
        demonstração das equações normais, na nota de aula.
  \item Mostre que, para todo $\lambda>0$, a matriz
        $\mathbb{X}^\top\mathbb{X}+\lambda\mathbb{I}$ é invertível --- \textbf{mesmo quando
        $p>n$}, caso em que o MQO nem está definido.
...
\textbf{(a)} Expandindo,
\[
  S_\lambda(\bbeta)=\bm y^\top\bm y-2\bbeta^\top\mathbb{X}^\top\bm y
                   +\bbeta^\top\mathbb{X}^\top\mathbb{X}\bbeta+\lambda\bbeta^\top\bbeta,
\]
cujo gradiente é
$\nabla S_\lambda(\bbeta)=-2\mathbb{X}^\top\bm y+2\mathbb{X}^\top\mathbb{X}\bbeta+2\lambda\bbeta$.
Igualando a zero:
\[
  (\mathbb{X}^\top\mathbb{X}+\lambda\mathbb{I})\,\bbeta=\mathbb{X}^\top\bm y .
\]
Pelo item (b) a matriz é invertível, então a solução é única e vale
$(\mathbb{X}^\top\mathbb{X}+\lambda\mathbb{I})^{-1}\mathbb{X}^\top\bm y$. A Hessiana
$2(\mathbb{X}^\top\mathbb{X}+\lambda\mathbb{I})$ é definida positiva, logo é mínimo.
...
invertível. Note onde a hipótese $p>n$ \emph{não} foi usada: a conta não depende
de $\mathbb{X}$ ter posto cheio. Se $p>n$, existe $\bm v\neq\bm 0$ com
$\mathbb{X}\bm v=\bm 0$ --- e é justamente aí que a primeira parcela morre e a segunda
salva a conta.

\smallskip
\textbf{(c)} Quando $p>n$ o MQO não tem solução única: a matriz
$\mathbb{X}^\top\mathbb{X}$ é singular e existem infinitos $\bbeta$ com o mesmo erro de
