...
\begin{equation}\label{eq:ridgesol}
  \widehat{\bbeta}^{\,\text{ridge}}
   = (\mathbb{X}^\top\mathbb{X} + \lambda \mathbb{I})^{-1}\mathbb{X}^\top\bm{y}.
\end{equation}
...
de menos. Isso conecta diretamente com a Aula~06 (\emph{pipelines} e
...
\begin{itemize}[leftmargin=1.4cm]
  \item Regressão linear: $g(\x)=\bbeta^\top\x$, linear \emph{nos parâmetros};
        MQO $\widehat{\bbeta}=(\mathbb{X}^\top\mathbb{X})^{-1}\mathbb{X}^\top\bm{y}$.
