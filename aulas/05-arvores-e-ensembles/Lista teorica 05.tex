...
%==============================================================================
%  Lista Teórica 05 --- Árvores de Regressão e Ensembles
...
\begin{document}
\cabecalholista{05}{Árvores de Regressão e \emph{Ensembles}}
...
%------------------------------------------------------------------------------
\begin{exercicio}[por que $B$ grande não estraga]
No \emph{bagging} e nas florestas aleatórias, cada árvore é ajustada a uma
reamostra dos dados, \textbf{sem olhar} para as árvores anteriores. Aumentar $B$
pode piorar o desempenho fora da amostra? Justifique usando a fórmula da variância
da média.
\end{exercicio}

\begin{solucao}
\textbf{Não} --- ou, mais precisamente, não de forma sistemática. As
...
O ganho de descorrelacionar é, portanto, \textbf{sete vezes maior} --- e a
diferença que mais importa não é de tamanho, é de natureza: o efeito de $B$
\textbf{se esgota} e o de $\rho$ não. Com $\rho=0{,}6$ fixo, nenhum $B$ leva a
variância abaixo de $0{,}60$, por maior que seja o orçamento; baixar $\rho$ pela
metade atravessa esse piso, que $B$ sequer encosta. É por isso que a floresta
aleatória é uma ideia melhor do que simplesmente rodar mais árvores.
...
qualidade individual é mais que compensada pela queda de $\rho$ --- e a conta
do item (c) mostra por que a aposta costuma valer a pena.

As \emph{extremely randomized trees} levam a ideia adiante por \emph{outro} eixo:
além de sortear as covariáveis candidatas, sorteiam o \textbf{limiar} de cada uma
em vez de procurar o melhor corte --- mais viés ainda, e menos $\rho$ ainda. Os
dois sorteios são independentes, e é só no extremo em que se juntam ($m=1$ com
limiar ao acaso) que a árvore fica inteiramente aleatória.
\end{solucao}
