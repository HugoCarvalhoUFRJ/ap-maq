...
%  (O roteiro de sala é "05 Arvores de Regressao e Ensembles.tex".)
%
...
um dos métodos \emph{mais poderosos} que existem para dados tabulares: as
florestas aleatórias. A pergunta que organiza tudo:
...
prática 05} mede o que a árvore gananciosa faz com ele:

\begin{itemize}[leftmargin=1.2cm,nosep]
  \item o melhor corte da raiz reduz $0{,}194\%$ do RSS dali, e nos dois filhos que
        ele produz os melhores cortes reduzem $0{,}447\%$ e $0{,}942\%$ --- a cegueira
        \textbf{não} é só do primeiro passo, porque os oito octantes continuam se
        cancelando dois a dois depois de qualquer corte. Para ter escala, a partição
        verdadeira em oito octantes deixa RSS zero;
  \item o RSS de teste fica \textbf{acima} do de chutar a média até a
        profundidade~5, e pior a cada nível --- a árvore gasta folhas para ficar
        mais errada que o preditor constante; cai para $83\%$ dele na~6 e só chega
        a $13\%$ com a árvore cheia, de 59 folhas.
\end{itemize}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.62\textwidth]{../../recursos/figuras/05-numero-arvores.pdf}
  \caption{Por que $B$ não é um \emph{tuning parameter} na floresta, medido num
  problema com $n=150$, $p=8$ e ruído alto. Com $B=1$ --- uma árvore só --- o risco é
  $8{,}04$; ele desaba, estabiliza em torno de $3{,}34$ e \emph{fica lá}: de $B=100$
  para $B=500$ varia $-0{,}4\%$. O erro de treino também estaciona, em $0{,}47$, em
  vez de descer rumo a zero --- cada árvore nova é ajustada a uma reamostra
  independente, e a média de mais árvores independentes não persegue ruído. Você pode
  exagerar em $B$ à vontade; só perde tempo de máquina.}
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \item \textbf{Florestas}: \emph{descorrelacionam} sorteando $m\approx p/3$
        variáveis por nó; adaptam-se à esparsidade; e são \emph{robustas a $B$} ---
        exagerar só custa tempo de máquina (Fig.~\ref{fig:numeroarvores}).
\end{itemize}

\paragraph{Leitura recomendada.} [AME] §4.8 (árvores, com o algoritmo de
crescimento e poda) e §4.9 (\emph{bagging} e florestas aleatórias). [ISLP] §8.1,
que tem a versão deles da nossa Figura~\ref{fig:particao}, e §8.2.1--8.2.2 sobre
\emph{bagging} e florestas.
